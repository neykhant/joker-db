...
import { getItemName, editItemName } from "../../store/actions";
...
const EditItemName = ({
  ...
  useEffect(() => {
    form.setFieldsValue({ name: itemName.itemName.name });
  }, [form, itemName.itemName]);

  const onFinish = async (values) => {
    await editItemName(param.id, values);
  };
      ...
      <Layout style={{ margin: "20px" }}>
        <Space direction="vertical" size="middle">
          <Title style={{ textAlign: "center" }} level={3}>
            ပစ္စည်းအမည် ပြင်ဆင်ခြင်း စာမျက်နှာ
          </Title>
          <Form
            labelCol={{
              xl: {
...
export default connect(mapStateToProps, { getItemName, editItemName })(
  EditItemName
);
